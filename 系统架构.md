...
| 安全 | helmet、cors、express-rate-limit、token_version、校园网/VPN 访问白名单 |
...
未来部署到学校云服务器时，推荐在 Nginx 或云安全组前增加校园网/VPN 访问边界：

```mermaid
flowchart TD
  User["用户设备"] --> Access["校园网 / VPN"]
  Access --> Nginx2["学校云服务器 Nginx<br/>HTTPS + IP 白名单"]
  Nginx2 --> Node2["Node/Express<br/>127.0.0.1:3000"]
  Node2 --> DB2["MySQL<br/>127.0.0.1:3306"]
  Node2 --> Files2["uploads 文件目录"]
  Nginx2 -.deny all.-> Files2
```
...
| 校园网/VPN 白名单 | 未来学校云部署时，由云安全组或 Nginx 限制访问来源，仅允许校园网出口 IP 和 VPN 出口 IP 访问 |
...
目标：系统部署在学校云服务器，数据库部署在同一台服务器或校内内网数据库中。用户必须处于校园网网段，或通过学校 VPN 接入后，才能访问和登录系统。

推荐策略：

1. 云安全组只向校园网出口 IP 段和 VPN 出口 IP 段放行 80/443。
2. Nginx 配置 `allow`/`deny` 作为第二层访问白名单。
3. Node 服务只监听 `127.0.0.1:3000`，由 Nginx 反向代理访问。
4. MySQL 只监听 `127.0.0.1:3306` 或校内内网地址，不开放公网访问。
5. `uploads` 目录不提供静态访问，继续通过受权限校验的下载接口读取。
6. SSH 管理入口也建议限制为校园网/VPN 或管理员固定 IP。
...
server {
    listen 443 ssl;
    server_name tw.example.edu.cn;

    # 替换为学校信息化部门提供的校园网出口 IP 段和 VPN 出口 IP 段
    # 如果云服务器看到的是学校公网出口 IP，应填写公网出口段；
    # 如果部署在校内私有云，才使用对应的内网网段。
    allow 10.0.0.0/8;
    allow 172.16.0.0/12;
...
1. 校园网出口公网 IP 段。
2. VPN 出口 IP 段。
3. 云服务器安全组、防火墙或网关白名单配置方式。
4. 系统域名、HTTPS 证书和是否需要 HSTS。
5. 是否需要校内审批、备案或等保相关流程。
6. 数据库和上传文件的备份周期、保留期限与恢复要求。

## 14. 最近更新

### 2026-05-29

- 补充学校云服务器部署规划，支持仅校园网网段或 VPN 接入后访问系统。
- 明确 MySQL 不暴露公网，推荐由云安全组和 Nginx 双层白名单控制访问来源。
- 增加上线前需要向学校确认的网络、域名、证书、审批和备份事项。
